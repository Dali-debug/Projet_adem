#!/usr/bin/env python3
#
# Test a disaggregation algorithm (test_Algorithm.py)
...
for (fold, priors, testing) in folds: 
    ...
    # Initialize lists to store predictions and truths for plotting
    all_y_true = []
    all_y_est = []
    ...
    # Generate plots for last fold only
    if fold == folds.folds - 1:
        timesteps = range(len(all_y_est))
        
        # Plot for each appliance
        for idx, label in enumerate(labels):
            if label == 'UNE':  # Skip noise component
                continue
            
            plt.figure(figsize=(10,6))
            
            # Extract predicted and true values for current appliance
            predicted = [y[idx] for y in all_y_est]
            actual = [y[idx] for y in all_y_true]
            
            plt.plot(timesteps, actual, label='Actual', alpha=0.7)
            plt.plot(timesteps, predicted, label='Predicted', alpha=0.7)
            
            plt.xlabel('Time (samples)')
            plt.ylabel(f'Power ({measure})')
            plt.title(f'Actual vs Predicted power ({measure}) for {label}')
            plt.legend()
            plt.grid(True)
            
            plt.savefig(f'{test_id}_{label}_plot.png')
            plt.close()
